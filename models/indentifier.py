...
def check_image_dtype_and_shape(image):
    if not isinstance(image, np.ndarray):
        raise Exception("image is not np.ndarray!")
...
class PlantIdentifier(OnnxModel):
    def __init__(self, model_dir=None):
        if model_dir is None:
            ...
            model_dir = os.path.join(current_dir, "pretrained-models")
        ...
        self.names = [value["latin_name"] for value in self.label_map["species_taxons"].values()]
    ...
    def _preprocess(image):
        ...
        image = resize_image_short(image, 224)
        image = center_crop_image(image, 224, 224)
        image = normalize_image_channel(image, swap_rb=True)
        mean, stddev = [0.485, 0.456, 0.406], [0.229, 0.224, 0.225]
        image = normalize_image_value(image, mean, stddev, "auto")
    ...
    def predict(self, image):
        ...
        family_probs = sum_by_indices_list(probs, self.family_class_indices, axis=-1)
        genus_probs = sum_by_indices_list(probs, self.genus_class_indices, axis=-1)
    ...
    def identify(self, image, topk=5):
        ...
        family_topk_probs, family_topk_indices = top_k(family_probs, family_topk)
        for ind, prob in zip(family_topk_indices[0], family_topk_probs[0]):
            one_result = OrderedDict()
            one_result["name"] = self.family_names[ind]
        ...
        genus_topk_probs, genus_topk_indices = top_k(genus_probs, genus_topk)
        for ind, prob in zip(genus_topk_indices[0], genus_topk_probs[0]):
            one_result = OrderedDict()
            one_result["name"] = self.genus_names[ind]
